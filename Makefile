...
help:
	...
	@echo "  eval       Run the full Bifrost pipeline through MTEB AppsRetrieval"
...
baseline:
	...
	python -m bifrost.eval.run_mteb --model e5-base-v2 --mode baseline --out $(OUT)/baseline
	@echo "Finished at $$(date)"

# Full pipeline. Produces appsretrieval_results.json for the Release asset.
eval:
	@echo "Starting eval run at $$(date)"
	python -m bifrost.eval.run_mteb --config $(CONFIG) --out $(OUT)/bifrost
	@echo "Finished at $$(date)"

# Every pipeline stage must earn its place in this table.
ablate:
	python -m bifrost.eval.ablate --config $(CONFIG) --out $(OUT)/ablation

# Build or incrementally update the index for a repository.
#   make index REPO=../some-js-repo VERSION=HEAD
index:
	python -m bifrost.indexing.build --repo $(REPO) --version $(VERSION) --config $(CONFIG)

demo:
	python -m bifrost.api.server --config $(CONFIG) &
